...
def main():
    ...
    if len(balance):
        cols=['smd_wm_distance','smd_gm_probability']; balance['all_covariates_balanced']=balance[cols].abs().max(axis=1)<=MAX_ABSOLUTE_SMD
        eligible=balance.loc[balance.n_matched.ge(MIN_MATCHED_VOXELS) & ((balance.all_covariates_balanced) if REQUIRE_BALANCE else True),['subject','tract','endpoint']].rename(columns={'tract':'tract_name'})
