...
INPUT_DIR='/bml/projects/06_resilience/projects/06-12_wm-gm-fc-connectome/derivatives/pt2_group_results/no8_endpoint_covariates'
OUTPUT_DIR='/bml/projects/06_resilience/projects/06-12_wm-gm-fc-connectome/derivatives/pt2_group_results/no9_matched_endpoint_fc'
...
def main():
    ...
    for subj in subjects:
        z=np.load(os.path.join(INPUT_DIR,f'{subj}_aim2_endpoint_covariates.npz'),allow_pickle=True);cov=np.column_stack([z['wm_boundary_distance'],z['gm_probability']]);base=os.path.join(BATCH_DIR,subj);b=nib.load(os.path.join(base,'gm','func','derived',f'bnbmdenoised_detrended_rest_{subj}.nii'));d=b.get_fdata();gm=d[...,0]>0 if d.ndim==4 else d>0;n=int(gm.sum());lab=nib.load(os.path.join(base,'gm','derived',f'fmri_wsst_all_ROIs_AAL_u_rc1sub-{subj}_ses-1_T1w_NCKU_336Ss.nii')).get_fdata().astype(int)[gm];regions={r:np.flatnonzero(lab==r) for r in np.unique(lab) if r>0};R=np.load(os.path.join(base,'gm','func','derived','fc_results',f'{subj}_R.npz'))['arr_0'];W=np.load(os.path.join(base,'gm','anat','derived','dist_results',f'{subj}_W_a.npz'))['arr_0'];ii,jj=np.triu_indices(n,1);R=R[ii,jj] if R.ndim==2 else R;W=W[ii,jj] if W.ndim==2 else W;rows=[];balance=[]
        for k,tract in enumerate(z['tract']):
            ...
            balance.append({'subject':subj,'tract':tract,'endpoint':int(z['endpoint'][k]),'n_matched':len(e),'mean_match_distance':np.mean(md),'smd_wm_distance':smd(cov[e,0],cov[c,0]),'smd_gm_probability':smd(cov[e,1],cov[c,1])})
