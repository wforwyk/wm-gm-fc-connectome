#!/usr/bin/env python3
# =============================================================================
# pt2_no4_endpoint_voxel_covariates_v2.py
#
...
#   GM probability (SPM c1), and tSNR.  All images must share the BnB grid.
# INPUT: BnB mask, AAL image, aal_summary.csv, endpoint images, and 3 maps.
# OUTPUT: {subj}_aim2_endpoint_covariates.npz; aim2_no4_v2_qc.csv.
...
OUTPUT_DIR = '/bml/projects/06_resilience/projects/06-12_wm-gm-fc-connectome/derivatives/pt2_group_results/no4_endpoint_covariates_v2'
TRACT_SUFFIX = '_avg16_syn_bbr'
QC_EXCLUDE = {'rh.atr', 'acomm'}
ENDPOINT_GLOB_TEMPLATE = '{batch_dir}/{subject}/wm/derived/endpoints/endpt{end}/{tract}.nii.gz'
...
WM_DISTANCE_TEMPLATE = '{batch_dir}/{subject}/gm/anat/derived/wm_boundary_distance.nii.gz'
GM_PROB_TEMPLATE = '{batch_dir}/{subject}/gm/anat/derived/c1_gm_probability.nii.gz'
TSNR_TEMPLATE = '{batch_dir}/{subject}/gm/func/derived/tsnr.nii.gz'
...
def main():
    ...
    for subj in subjects:
        base=os.path.join(BATCH_DIR,subj); bfile=os.path.join(base,'gm','func','derived',f'bnbmdenoised_detrended_rest_{subj}.nii'); afile=os.path.join(base,'gm','derived',f'fmri_wsst_all_ROIs_AAL_u_rc1sub-{subj}_ses-1_T1w_NCKU_336Ss.nii'); sfile=os.path.join(base,'wm','derived','aal_summary',f'{subj}_aal_summary.csv')
        maps={'wm_boundary_distance':path(WM_DISTANCE_TEMPLATE,subject=subj),'gm_probability':path(GM_PROB_TEMPLATE,subject=subj),'tsnr':path(TSNR_TEMPLATE,subject=subj)}
    ...
    pd.DataFrame(qc).to_csv(os.path.join(OUTPUT_DIR,'aim2_no4_v2_qc.csv'),index=False)
