...
INPUT   : dwi_aal      — dwi_wsst_all_ROIs_AAL_u_rc1sub-{subj}_ses-1_T1w_NCKU_336Ss.nii
          ...
          fc           — {subj}_R.npz
          dist         — {subj}_W_a.npz
          c1           — gm/anat/segmentation/  (rc1*.nii)
...
NOTE    : Numeric subject IDs only (no sub- prefix) for batch2.
          voxelpair CSV (idx_u, idx_v, RD) is required by no11.
...
def build_region_groups(fmri_aal_path, c1_dir, lut):
    aal_data = nib.load(fmri_aal_path).get_fdata().astype(np.int32)
    c1_files = sorted(Path(c1_dir).glob("rc1*.nii"))
    c1_data  = nib.load(str(c1_files[0])).get_fdata()
    vset     = np.where(c1_data.flatten() > 0)[0]
    aal_ids  = aal_data.flatten()[vset]
    groups   = {}
    for row_idx, aid in enumerate(aal_ids):
        name = lut.get(int(aid), "NA")
        if name != "NA":
            groups.setdefault(name, []).append(row_idx)
    print(f"    fMRI vset: {len(vset):,}   AAL regions: {len(groups)}")
    return groups
...
def run_subject(subj, lut):
    ...
    c1        = f"{BASE_DIR}/{subj}/gm/anat/segmentation"
    ...
    print("  Building fMRI region groups...")
    region_groups = build_region_groups(fmri_aal, c1, lut)
    ...
    BASE_DIR_fc    = np.concatenate(fc_list)
    BASE_DIR_dist  = np.concatenate(dist_list)
    BASE_DIR_tract = np.concatenate(tract_list)
    BASE_DIR_idx_u = np.concatenate(idx_u_list)
    BASE_DIR_idx_v = np.concatenate(idx_v_list)
    print(f"  BASE_DIR voxel pairs: {len(BASE_DIR_fc):,}")

    for metric in METRICS:
        print(f"\n  [{metric}]")
        metric_col = f"{metric}_Avg"
        tract_metric = {}
        for tract_name in np.unique(BASE_DIR_tract):
        ...
        wm_vals = np.array([tract_metric.get(t, np.nan) for t in BASE_DIR_tract])
        valid   = np.isfinite(wm_vals)

        df = pd.DataFrame({
            "fc"    : BASE_DIR_fc[valid],
            "dist"  : BASE_DIR_dist[valid],
            metric  : wm_vals[valid],
            "tract" : BASE_DIR_tract[valid],
            "idx_u" : BASE_DIR_idx_u[valid],
            "idx_v" : BASE_DIR_idx_v[valid],
        })
