...
- Python 3 with `numpy`, `scipy`, `pandas`, `nibabel`, and `matplotlib`
  (`plotly` and `statsmodels` are optional for selected outputs)
...
Part 1, per subject
  ...
  Template: pt1_template_no00 → no01 (manual, once) → no02 → no03 → no04 → no05
...
input/output path. In particular, visually verify DWI-to-fMRI alignment before
reslicing endpoint maps, inspect endpoint threshold sensitivity, and review
distance confounding in WM-present versus WM-absent comparisons. Part 2 is
active research; its outputs require study-specific QC and statistical review.
...
part2/            group-level analysis and figure scripts
```
