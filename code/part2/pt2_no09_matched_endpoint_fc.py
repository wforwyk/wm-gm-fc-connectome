...
def main():
    ...
    for subj in subjects:
        z=np.load(os.path.join(INPUT_DIR,f'{subj}_aim2_endpoint_covariates.npz'),allow_pickle=True);cov=np.column_stack([z['wm_boundary_distance'],z['gm_probability']]);base=os.path.join(BATCH_DIR,subj);b=nib.load(os.path.join(base,'gm','func','derived',f'bnbmdenoised_detrended_rest_{subj}.nii'));d=b.get_fdata();gm=d[...,0]>0 if d.ndim==4 else d>0;n=int(gm.sum());lab=nib.load(os.path.join(base,'gm','derived',f'fmri_wsst_all_ROIs_AAL_u_rc1sub-{subj}_{SOURCE_SESSION}_T1w_{DARTEL_TEMPLATE_ID}.nii')).get_fdata().astype(int)[gm];regions={r:np.flatnonzero(lab==r) for r in np.unique(lab) if r>0};R=np.load(os.path.join(base,'gm','func','derived','fc_results',f'{subj}_R.npz'))['arr_0'];W=np.load(os.path.join(base,'gm','anat','derived','dist_results',f'{subj}_W_a.npz'))['arr_0'];ii,jj=np.triu_indices(n,1);R=R[ii,jj] if R.ndim==2 else R;W=W[ii,jj] if W.ndim==2 else W;rows=[];balance=[]
