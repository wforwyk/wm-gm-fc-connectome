...
def main():
    ...
    u=d.groupby(['subject','tract_name','endpoint','source_roi','voxel_class','target_type'],as_index=False).agg(mean_fc_z=('mean_fc_z','mean'),fc_distance_log_slope=('fc_distance_log_slope','mean'),wm_boundary_distance=('wm_boundary_distance','mean'),gm_probability=('gm_probability','mean'),tsnr=('tsnr','mean'))
    ...
    if len(balance):
        cols=['smd_wm_distance','smd_gm_probability','smd_tsnr'];balance['all_covariates_balanced']=balance[cols].abs().max(axis=1)<=MAX_ABSOLUTE_SMD;balance.to_csv(os.path.join(OUTPUT_DIR,'aim2_matching_balance_all.csv'),index=False)
    ...
    try:
        ...
        fit=smf.ols('mean_fc_z ~ endpoint_binary*distant_binary + wm_boundary_distance + gm_probability + tsnr + C(subject)',data=m).fit(cov_type='cluster',cov_kwds={'groups':m['subject']})
